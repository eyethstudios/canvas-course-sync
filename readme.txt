...
= 3.5.2 =
* Reverted all code changes back to stable 3.5.2 version
* Removed complex badge functionality that was causing 500 server errors
* Simplified content handler to basic functionality only
* Plugin now works with simple course content generation without badges
...
* Corrected badge image paths for web accessibility and display
...
= 3.4.1 =
* Fixed PHP syntax error in content handler
...
* Enhanced continuing education credit section to use Canvas course-specific data instead of generic defaults
* Improved course content display with cleaner formatting

= 3.2.5 =
* Enhanced content extraction to display actual Canvas course-specific information instead of generic placeholders
* Improved module description and learning objectives to pull real course content from Canvas
* Removed redundant course description display in favor of streamlined content presentation

= 3.2.3 =
* Fixed duplicate canvas_course_id error when re-syncing previously deleted courses
* Enhanced database transaction handling to update existing tracking records instead of inserting duplicates
* Improved course re-sync workflow with proper timestamp tracking and status management
...
* Improved course listing accuracy by consulting tracking table for sync status
