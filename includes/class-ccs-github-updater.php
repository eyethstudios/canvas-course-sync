...
class CCS_GitHub_Updater {
	...
	public function update_notice() {
		...
		if ( $this->is_update_available( $current_version, $remote_version ) ) {
			...
			echo '<p>' . sprintf(
				__( 'Canvas Course Sync version %1$s is available. <a href="%2$s">Update now</a>', 'canvas-course-sync' ),
	...
	public function ajax_check_updates() {
		...
		if ( $this->is_update_available( $current_version, $remote_version ) ) {
			...
			wp_send_json_success(
				array(
					'message'          => sprintf( __( 'Plugin is up to date! Current: %1$s, Latest: %2$s', 'canvas-course-sync' ), $current_version, $remote_version ),
