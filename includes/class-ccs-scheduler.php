...
class CCS_Scheduler {
	 ...
	 * @param CCS_Catalog_Api|null $catalogApi Catalog API instance (optional)
	 * @param CCS_Importer|null    $importer Importer instance (optional)
	 */
	public function __construct( CCS_Logger $logger = null, CCS_Catalog_Api $catalogApi = null, CCS_Importer $importer = null ) {
		$this->logger     = $logger ?: new CCS_Logger();
		$this->catalogApi = $catalogApi ?: new CCS_Catalog_Api( $this->logger );
	...
	public function run_auto_sync() {
		...
		try {
			...
			$canvas_courses = $this->catalogApi->get_courses();
	...
	private function find_new_courses( $canvas_courses ) {
		...
		foreach ( $canvas_courses as $course ) {
			...
			$exists_check = $this->db_manager->course_exists( $course['id'], $course['title'] );
			...
			if ( $exists_check['exists'] ) {
				...
				if ( $existing_post && $existing_post->post_status !== 'trash' ) {
					$this->logger->log( 'Skipping course "' . $course['title'] . '" - already exists and is active (' . $exists_check['type'] . ') - Post ID: ' . $exists_check['post_id'] );
				} else {
					// Course tracking exists but post is deleted/trashed - include for re-import
					$new_courses[] = $course;
					$this->logger->log( 'Course "' . $course['title'] . '" marked for re-import (tracking exists but post deleted) - Canvas ID: ' . $course['id'] );
				}
			} else {
				$new_courses[] = $course;
				$this->logger->log( 'Course "' . $course['title'] . '" marked for import (Canvas ID: ' . $course['id'] . ')' );
			}
		}
