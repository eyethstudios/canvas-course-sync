...
class CCS_Database_Manager {
	...
	private function maybe_create_course_tracking_table() {
		...
		$sql = "CREATE TABLE IF NOT EXISTS $table_name (
            ...
            UNIQUE KEY catalog_course_id (catalog_course_id),
	...
	public function course_exists( $catalog_id, $course_title = '' ) {
		...
		error_log( 'CCS_Database_Manager: Checking if course exists - Catalog ID: ' . $catalog_id . ', Title: ' . $course_title );

		// Check custom tracking table first
		$tracking_table    = $wpdb->prefix . 'ccs_course_tracking';
		$existing_tracking = $wpdb->get_row(
			$wpdb->prepare(
		...
		$existing_by_catalog_id = $wpdb->get_row(
			$wpdb->prepare(
		...
		if ( $existing_by_catalog_id ) {
			error_log( 'CCS_Database_Manager: Course found by Catalog ID in WordPress: ' . print_r( $existing_by_catalog_id, true ) );
			return array(
				'exists'  => true,
				'type'    => 'wordpress_meta',
		...
		if ( ! empty( $course_title ) ) {
			$existing_by_title = $wpdb->get_row(
				$wpdb->prepare(
	...
	public function create_course_with_transaction( $course_data ) {
		...
		try {
			...
			$existing_tracking = $wpdb->get_row(
				$wpdb->prepare(
					"SELECT * FROM $tracking_table WHERE catalog_course_id = %d OR WHERE canvas_course_id = %d",
			...
			if ( $existing_tracking ) {
				...
				error_log( 'CCS_Database_Manager: Updating existing tracking record for Catalog ID: ' . $course_data['catalog_id'] );
				...
				$tracking_result = $wpdb->update(
					...
					array( 'catalog_course_id' => intval( $course_data['catalog_id'] ) ),
					array( '%d', '%s', '%s', '%s', '%s' ),
					array( '%d', '%d' )
				);
				...
				error_log( 'CCS_Database_Manager: Inserting new tracking record for Catalog ID: ' . $course_data['catalog_id'] );
				...
				$tracking_result = $wpdb->insert(
					...
					array(
						...
						'course_title'             => sanitize_text_field( $course_data['title'] ),
						'course_slug'              => sanitize_title( $course_data['slug'] ?? '' ),
						'course_description'       => sanitize_text_field( $course_data['description'] ?? '' ),
						'course_short_description' => sanitize_text_field( $course_data['short_description'] ?? '' ),
						'sync_status'              => 'synced',
					),
			...
			if ( $should_update ) {
				...
				if ( $update_result !== false ) {
					...
					$results['details'][] = array(
						...
						'catalog_id'   => $tracked_course->catalog_course_id,
					...
					if ( $this->logger ) {
						$this->logger->log( "Updated course sync status to 'available': {$tracked_course->course_title} (Catalog ID: {$tracked_course->catalog_course_id}) - Post was {$status}" );
					}
				} else {
